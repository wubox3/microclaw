...
import {
  createGccProgrammingSkillsManager,
  formatProgrammingSkillsForPrompt,
  hasApprovalSignal,
} from "./gcc-programming-skills.js";
...
describe("createGccProgrammingSkillsManager", () => {
  let db: DatabaseSync;
  ...
  describe("getSkills / saveSkills round-trip", () => {
    ...
    it("persists and retrieves skills via GCC", () => {
      const store = createGccStore(db);
      const mgr = createGccProgrammingSkillsManager(db, store);
    ...
    it("uses in-memory cache after first read", () => {
      ...
      db.prepare("DELETE FROM gcc_commits WHERE memory_type = 'programming_skills'").run();
      db.prepare("UPDATE gcc_branches SET head_commit_hash = NULL WHERE memory_type = 'programming_skills'").run();
  ...
  describe("extractAndUpdateSkills", () => {
    ...
    it("extracts and saves skills from exchanges (verify GCC commit created)", async () => {
      seedExchanges(db, [
        {
      ...
      // Verify GCC commit was created
      const logEntries = store.log("programming_skills");
      expect(logEntries.length).toBeGreaterThanOrEqual(1);
      expect(logEntries[0].message).toContain("Extracted from recent conversations");
    });
