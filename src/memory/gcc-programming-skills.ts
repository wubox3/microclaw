...
const log = createLogger("gcc-programming-skills");

export type GccProgrammingSkillsManager = {
...
export function createGccProgrammingSkillsManager(
  db: SqliteDb,
  gccStore: GccStore,
): GccProgrammingSkillsManager {
  ...
  const getSkills = (): ProgrammingSkills | undefined => {
    ...
    const snapshot = gccStore.getHeadSnapshot("programming_skills");
    if (!snapshot) {
    ...
    try {
      cachedSkills = snapshot as unknown as ProgrammingSkills;
      return structuredClone(cachedSkills);
    } catch {
      log.warn("Failed to read programming skills from GCC store");
      cachedSkills = undefined;
      return undefined;
    }
  };
  ...
  const saveSkills = (skills: ProgrammingSkills): void => {
    const confidence = skills.approvedPatterns.length > 0 ? "HIGH_CONFIDENCE" : "MEDIUM_CONFIDENCE";
    gccStore.commit({
      memoryType: "programming_skills",
      snapshot: skills as unknown as Record<string, unknown>,
      message: "Manual save",
      confidence,
    });
  ...
  const extractAndUpdateSkills = async (
    ...
    try {
      ...
      const hasApprovedContent = merged.approvedPatterns.length > 0;
      gccStore.commit({
        memoryType: "programming_skills",
        snapshot: merged as unknown as Record<string, unknown>,
        message: "Extracted from recent conversations",
        confidence: hasApprovedContent ? "HIGH_CONFIDENCE" : "MEDIUM_CONFIDENCE",
      });

      cachedSkills = structuredClone(merged);
      log.info("Programming skills updated via GCC commit");
    } catch (err) {
      log.warn(
