...
const log = createLogger("skills:watch");
...
function emit(event: SkillsChangeEvent): void {
...
function resolveWatchPaths(workspaceDir: string, config?: EClawConfig): string[] {
  const paths: string[] = [];
  if (workspaceDir.trim()) {
    paths.push(path.join(workspaceDir, "skills"));
  }
  paths.push(path.join(ECLAW_CONFIG_DIR, "skills"));
